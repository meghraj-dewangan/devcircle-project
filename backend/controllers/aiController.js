...
const getAiError=(error)=>{
  ...
  if (
    message.includes('empty response') ||  message.includes('short response') ||  message.includes('oversized response') || message.includes('boilerplate content') ||  message.includes('unrelated post') ||  message.includes('Unable to parse model response')
   
  ) {
    return { status: 502, message: 'AI an invalid draft. Please try again.' };
  }
